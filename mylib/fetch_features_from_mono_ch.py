...
def FetchFeaturesFromMonoData2(v: List, fs: int, N: int, overlap: Union[int, float], th: Union[int, float] = 7000, w: int = 3, sec_range: float = 0.01, nth: int = 9):
    ...
    # FFTを用いた周波数解析
    fft_array, fft_mean, fft_axis = fft(v, fs, N, overlap)
    ...
    ratio_max_to_10ms_ave_peaks = fff.GetRatioMaxToOtherAvePeaks(v, w, fs, sec_range)
    ...
    return [low_power, high_power, hlbr, ratio_max_to_10ms_ave_peaks, ratio_max_to_9th_ave_peaks, ac_std, ac_auc, diff_std, diff_auc, srmr_val]
                            ...
                            for utterance_id in utterance_ids:
                                ...
                                for dov_angle in dov_angles:
                                    ...
                                    for ch in gp_tdoa_mic_channels:
                                        fname = utterance_id + '_' + \
                                            dov_angle + '_' + str(ch) + '.wav'
                                        fpath = DATASET_PATH + '/' + first_dir_name + '/' + \
                                            second_dir_name + '/' + third_dir_name + '/' + fname
                                    ...
                                    # 各ペアについて計算
                                    mic_ch_pairs = itertools.combinations(
                                        gp_tdoa_mic_channels, 2)
                                    ...
                                    gp_tdoa_features = []
                                    np_pair_gp_tdoa_features = np.array(
                                        pair_gp_tdoa_features)
                                    for i in range(4):
...
def FetchFeaturesFromDataset2(DATASET_PATH) -> List:
    '''
    データセットの音声データから特徴量を抽出する関数
                        ...
                        for polar_angle_ix, polar_angle_id in enumerate(polar_angle_ids):
                            ...
                            for utterance_id in utterance_ids:
                                ...
                                for dov_angle in dov_angles:
                                    ...
                                    N = 2**12
                                    # 特徴量を得る
                                    features = FetchFeaturesFromMonoData2(v=v, fs=fs, N=N,w=1)
